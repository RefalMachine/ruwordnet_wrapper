...
class WordNet:
    ...
    def remove_synsets_by_words(self, words):
        for w in words:
...
class RuWordNet(WordNet):
    def __init__(self, wordnet_root, convert_instance_rels=True):
        super().__init__()
        self._load_wordnet(wordnet_root, convert_instance_rels=convert_instance_rels)

    def _load_wordnet(self, wordnet_root, convert_instance_rels=True):
        self._load_synsets(wordnet_root)
        self._load_rels(wordnet_root, convert_instance_rels=convert_instance_rels)
    ...
    def _load_rels(self, wordnet_root, convert_instance_rels=True):
        synsets_rels_paths = {
        ...
        for _, synset_rels_path in synsets_rels_paths.items():
